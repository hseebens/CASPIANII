...
#
# In diesem Skript werden taxonomische Namen mittels der GBIF backbone
# taxonomy standardisiert. Synonyme werden mit akzeptierten Taxonnamen ersetzt.
# 
...
CheckGBIFTax <- function(dat){
  ...
  if ("TaxonName"%in%colnames(dat)) colnames(dat)[colnames(dat)=="TaxonName"] <- "Taxon_orig"
  ...
  for (j in 1:n_taxa){# loop over all species names; takes some hours...
    ...
    # db_all <- name_backbone_verbose("Oenone fulgida (Savigny, 1818)",strict=T) # check for names and synonyms
    db <- db_all[["data"]]
    ...
    if (any(db$status=="ACCEPTED" & db$matchType=="EXACT" & colnames(db)=="canonicalName")){ 
      ...
      if (any(alternatives$status=="ACCEPTED" & alternatives$matchType=="EXACT")){
        ...
        dat$GBIFstatus_Synonym[ind_tax] <- alternatives[alternatives$status=="ACCEPTED" & alternatives$matchType=="EXACT",]$status[1]
    ...
    } else if (any(alternatives$status=="ACCEPTED" & alternatives$matchType=="EXACT" & any(colnames(db)=="species"))) {
