...
ueberpruefe_Datenverzeichnisse <- function (){
  ...
  if (any(file.exists(file.path("ListeNeobiota","Data","Input","ListeGebietsfremderArten_einzelneDB_Rohdaten.xlsx"))==FALSE)){
    stop(paste0("\n Datensatz ListeGebietsfremderArten_einzelneDB_Rohdaten.xlsx fehlt im Verzeichnis",file.path("ListeNeobiota","Data","Input")))
    y <- 1
  }
  
  ## check availability of borders of federal states
  if (any(file.exists(file.path("ListeNeobiota","Data","Input","List_IAS_union_concern.xlsx"))==FALSE)){
    stop(paste0("\n Datensatz List_IAS_union_concern.xlsx fehlt im Verzeichnis",file.path("ListeNeobiota","Data","Input")))
    y <- 1
  }
  
  ## check availability of borders of federal states
  if (any(file.exists(file.path("ListeNeobiota","Data","Input","skript519_checkliste.xlsx"))==FALSE)){
    stop(paste0("\n Datensatz skript519_checkliste.xlsx fehlt im Verzeichnis",file.path("ListeNeobiota","Data","Input")))
    y <- 1
  }
  
  ## check availability of borders of federal states
  if (any(file.exists(file.path("ListeNeobiota","Data","Input","PfadUebersetzung.xlsx"))==FALSE)){
    stop(paste0("\n Datensatz PfadUebersetzung.xlsx fehlt im Verzeichnis",file.path("ListeNeobiota","Data","Input")))
    y <- 1
  }
  
  ## check availability of borders of federal states
  if (any(file.exists(file.path("ListeNeobiota","Data","Input","INTRODUCTION_PATHWAYS.csv"))==FALSE)){
    stop(paste0("\n Datensatz INTRODUCTION_PATHWAYS.csv fehlt im Verzeichnis",file.path("ListeNeobiota","Data","Input")))
    y <- 1
  }
