...
standardisiereDaten <- function(Pfad_Datensaetze=Pfad_Datensaetze){
  ...
  for (i in 1:length(sheet_names)){#
    ...
    if ("status"%in%tolower(colnames(dat))){
      ...
      ind <- grepl("kryptog|cryptogen|unklar|nicht etabliert|fehlend|cryptogenic|questionable|unknown|unbestaendig",dat$status) # remove species with unclear, not established or cryptogenic status
      dat <- dat[!ind,]
    }
