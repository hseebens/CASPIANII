...
# Deutschland. Es wird mitdem Skript run_SDM_workflow.R aufgerufen.
#
# Das Skript bezieht für jede Art in StatusModellierung....xlsx Daten zum 
# Vorkommen auf online Platformen und eventuell eigenen Daten, sofern Daten 
# vorhanden sind. Das Skript ruft das Skript sammleVorkommenOnline.R auf, 
# welches die Verbindung zu den online Plattformen herstellt.
#
...
ermittleVorkommen <- function(TaxonName=NULL,
  ...
  if (x==1){
    ...
    cat(paste0("\n*** ",TaxonName," ist eine rein marine Art, fuer die keine Modellierung durchgeführt werden kann. ***\n") ) # notification for the user
    ...
    if (!is.null(Datenbank)){
      ...
      if (any(class(Vorkommen)=="try-error")){
        ...
        ## write status to log file
        status_species$Status[ind_species] <- "Keine Habitatmodellierung, da keine Vorkommensdaten bezogen werden konnten."
        
        ## export status of species list
        write.xlsx(status_species,file=file.path("SDM","Daten","Output",paste0("StatusModellierung",identifier,".xlsx",sep="")))
        
        print(paste0("Keine Vorkommensdaten auf online Plattformen für ", TaxonName,"."))        
        ...
        if (nrow(Vorkommen)>0){
          ...
          country_borders <- st_read(dsn=file.path("SDM","Daten","Input","Shapefiles"),layer="ne_50m_land",quiet=T)
    ...
    if (!is.null(Vorkommen_alle)){
      ...
      if (nrow(Vorkommen_alle) < min_limit) { # check, whether the number of occurrences is sufficient

        cat(paste0("\nDie Anzahl an Datenpunkten ist <", min_limit, ". Es kann keine Habitatmodellierung für ",TaxonName," durchgeführt werden.")) 
        ...
        ## save data to disk
        fwrite(Vorkommen_alle, file.path("SDM","Daten","Input",paste0("Vorkommen_",TaxonName,identifier,".csv"))) # stores the final occurrence file on the users computer
        
        cat(paste0("\n Vorkommensdaten wurden als 'Vorkommen_",TaxonName,".csv' im Verzeichnis 'Daten/Input' gespeichert.\n") ) # notification for the user
        
        ## write status to log file
        status_species$Status[ind_species] <- "Ausreichend Daten zur Habitatmodellierung vorhanden."
      ...
      cat(paste("\nEs liegen keine Daten zum Vorkommen von",TaxonName,"vor. Die Habitatmodellierung kann nicht durchgeführt werden.")) 
